...
function [Q] = Main_1gradient(lN,lG,lb,lk,lR)
...
while run<=Runs
    ...
    btypes=zeros(s,tau);                            % fecundity vector
    vtypes=zeros(s,tau);                            % activity vector
    MMb=zeros(1,tau);                               % population size 1
    MM=zeros(1,tau);                                % population size 2
...
dlmwrite(['N' num2str(N) 'G' num2str(G) 'be' num2str(beta) 'k' num2str(10*k1) 'r' num2str(10*r) '.txt'],Q)
end
